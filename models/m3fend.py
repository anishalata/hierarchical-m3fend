...
def scatter(batch_size, num, label):
    zeros = torch.zeros(batch_size, num)
    if torch.cuda.is_available():
        zeros = zeros.cuda()
    return zeros.scatter_(1, label, 1)
...
class MemoryNetwork(torch.nn.Module):
    def __init__(self, input_dim, emb_dim, domain_num, memory_num = 10):
        ...
        # Initialize domain memory for each domain
        self.domain_memory = {}
        for i in range(self.domain_num):
            domain_mem = torch.randn(self.memory_num, self.emb_dim)
            domain_mem = norm(domain_mem)
            if torch.cuda.is_available():
                domain_mem = domain_mem.cuda()
            self.domain_memory[i] = domain_mem

    def forward(self, feature, category):
        feature = norm(feature)
        domain_label = torch.tensor([index for index in category]).view(-1, 1)
        if torch.cuda.is_available():
...
class M3FENDModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        domain_indices = torch.LongTensor(range(self.domain_num))
        if torch.cuda.is_available():
            domain_indices = domain_indices.cuda()
        domain_emb_all = self.domain_embedder(domain_indices)
    ...
    def init_memory(self):
        for domain in range(self.domain_num):
            if domain in self.all_feature and len(self.all_feature[domain]) > 0:
                # If we have features for this domain
                all_feature = np.concatenate(self.all_feature[domain])
                n_clusters = min(self.memory_num, len(all_feature))
                kmeans = KMeans(n_clusters=n_clusters, init='k-means++').fit(all_feature)
                centers = kmeans.cluster_centers_
                
                # If we got fewer clusters than memory_num, pad with random values
                if n_clusters < self.memory_num:
                    padding = np.random.randn(self.memory_num - n_clusters, centers.shape[1])
                    centers = np.vstack([centers, padding])
            else:
                # If no features available, use random initialization
                centers = np.random.randn(self.memory_num, self.emb_dim)
            
            # Normalize the centers
            centers = centers / np.sqrt(np.sum(centers * centers, axis=-1, keepdims=True) + 1e-8)
...
class Trainer():
    ...
    def train(self, logger = None):
        ...
        self.model = M3FENDModel(self.emb_dim, self.mlp_dims, self.dropout, self.semantic_num, self.emotion_num, self.style_num, self.lnn_dim, len(self.category_dict), self.dataset)
        if self.use_cuda:
