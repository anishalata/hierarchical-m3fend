...
def word2input(texts, max_len, dataset):
    ...
    if dataset not in _tokenizer_cache:
        if dataset == 'ch':
            _tokenizer_cache[dataset] = BertTokenizer.from_pretrained('hfl/chinese-bert-wwm-ext')
        elif dataset == 'en':
            _tokenizer_cache[dataset] = RobertaTokenizer.from_pretrained('roberta-base')
    tokenizer = _tokenizer_cache[dataset]
    token_ids = []
