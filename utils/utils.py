...
def metrics(y_true, y_pred, category, category_dict):
    ...
    for c, res in res_by_category.items():
        if len(res['y_true']) > 0:
            metrics_by_category[c] = {
                'auc': round(float(roc_auc_score(res['y_true'], res['y_pred'])), 4)
            }
        else:
            metrics_by_category[c] = {
                'auc': 0.0
    ...
    for c, res in res_by_category.items():
        ...
        metrics_by_category[c] = {
            'precision': round(float(precision_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int), average='macro')), 4),
            'recall': round(float(recall_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int), average='macro')), 4),
            'fscore': round(float(f1_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int), average='macro')), 4),
            'auc': metrics_by_category[c]['auc'],
            'acc': round(float(accuracy_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int))), 4)
        }
